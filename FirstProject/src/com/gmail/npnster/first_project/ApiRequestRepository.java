...
public class ApiRequestRepository {
	...
	public UserParams getMyParams(Callback<UserParams> callback) {
		System.out.println(String.format("inside userParams,  user = %s, token = %s", mApp.getUserId(), mApp.getToken()));
	...
	public void onSignout(SignoutEvent event) {
		mRailsApi.signout(mApp.getToken(), mApp.getEmail(), "", new Callback<Void>() {
